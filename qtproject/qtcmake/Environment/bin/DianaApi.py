...
class DIANA_INTERFERENCE_ZONE_DOUBLE(Structure):
    pass

DIANA_INTERFERENCE_ZONE_DOUBLE._fields_ = [
    ('values', c_double * 3)
]
...
def getVelocityPercentValue(value, ipAddress=''):
    api_mod.getVelocityPercentValue.argtypes = [POINTER(c_int), c_char_p]
    api_mod.getVelocityPercentValue.restype = c_int
    ret = api_mod.getVelocityPercentValue(byref(value), bytes(ipAddress.encode('utf-8')))
    return ret

def modifyInterferenceZone(index, remark, diagonalName1, arrDiagonal1, diagonalName2, arrDiagonal2, arrBiased, tcpName, workPieceName, ipAddress=''):
    diagonal1 = DIANA_INTERFERENCE_ZONE_DOUBLE()
    diagonal1.values = (c_double*3)()
    for i in range(0, len(arrDiagonal1)):
        diagonal1.values[i] = arrDiagonal1[i]
    diagonal2 = DIANA_INTERFERENCE_ZONE_DOUBLE()
    diagonal2.values = (c_double*3)()
    for i in range(0, len(arrDiagonal2)):
        diagonal2.values[i] = arrDiagonal2[i]
    biased = DIANA_INTERFERENCE_ZONE_DOUBLE()
    biased.values = (c_double*3)()
    for i in range(0, len(arrBiased)):
        biased.values[i] = arrBiased[i]
    api_mod.modifyInterferenceZone.argtypes = [c_int, c_char_p, c_char_p, POINTER(DIANA_INTERFERENCE_ZONE_DOUBLE), c_char_p, POINTER(DIANA_INTERFERENCE_ZONE_DOUBLE), POINTER(DIANA_INTERFERENCE_ZONE_DOUBLE), c_char_p, c_char_p, c_char_p]
    api_mod.modifyInterferenceZone.restype = c_int
    ret = api_mod.modifyInterferenceZone(index, bytes(remark.encode('utf-8')), bytes(diagonalName1.encode('utf-8')), diagonal1, bytes(diagonalName2.encode('utf-8')), diagonal2, biased, bytes(tcpName.encode('utf-8')), bytes(workPieceName.encode('utf-8')), bytes(ipAddress.encode('utf-8')))
    return message(ret)

def getInterferenceZone(index, switch, remark, diagonalName1, arrDiagonal1, diagonalName2, arrDiagonal2, arrBiased, tcpName, workPieceName, ipAddress=''):
    diagonal1 = DIANA_INTERFERENCE_ZONE_DOUBLE()
    diagonal1.values = (c_double*3)()
    diagonal2 = DIANA_INTERFERENCE_ZONE_DOUBLE()
    diagonal2.values = (c_double*3)()
    biased = DIANA_INTERFERENCE_ZONE_DOUBLE()
    biased.values = (c_double*3)()
    strremark = create_string_buffer(256)
    strdiagonalName1 = create_string_buffer(256)
    strdiagonalName2 = create_string_buffer(256)
    strtcpName = create_string_buffer(256)
    strworkPieceName = create_string_buffer(256)

    api_mod.getInterferenceZone.argtypes = [c_int, POINTER(c_int), c_char_p, c_char_p, POINTER(DIANA_INTERFERENCE_ZONE_DOUBLE), c_char_p, POINTER(DIANA_INTERFERENCE_ZONE_DOUBLE), POINTER(DIANA_INTERFERENCE_ZONE_DOUBLE), c_char_p, c_char_p, c_char_p]
    api_mod.getInterferenceZone.restype = c_int
    ret = api_mod.getInterferenceZone(index, byref(switch), strremark, strdiagonalName1, diagonal1, strdiagonalName2, diagonal2, biased, strtcpName, strworkPieceName, bytes(ipAddress.encode('utf-8')))
    if(ret >= 0):
        for index in range(0, len(diagonal1.values)):
            arrDiagonal1[index] = diagonal1.values[index]
        for index in range(0, len(diagonal2.values)):
            arrDiagonal2[index] = diagonal2.values[index]
        for index in range(0, len(biased.values)):
            arrBiased[index] = biased.values[index]
        del remark[:]
        del diagonalName1[:]
        del diagonalName2[:]
        del tcpName[:]
        del workPieceName[:]
        remark.append(strremark.value.decode("utf-8"))
        diagonalName1.append(strdiagonalName1.value.decode("utf-8"))
        diagonalName2.append(strdiagonalName2.value.decode("utf-8"))
        tcpName.append(strtcpName.value.decode("utf-8"))
        workPieceName.append(strworkPieceName.value.decode("utf-8"))
    return message(ret)

def addInterferenceZoneIO(index, diordo, ioList, ipAddress=''):
    api_mod.addInterferenceZoneIO.argtypes = [c_int, c_int, c_char_p, c_char_p]
    api_mod.addInterferenceZoneIO.restype = c_int
    ret = api_mod.addInterferenceZoneIO(index, diordo, bytes(ioList.encode('utf-8')), bytes(ipAddress.encode('utf-8')))
    return message(ret)

def delInterferenceZoneIO(index, diordo, ioList, ipAddress=''):
    api_mod.delInterferenceZoneIO.argtypes = [c_int, c_int, c_char_p, c_char_p]
    api_mod.delInterferenceZoneIO.restype = c_int
    ret = api_mod.delInterferenceZoneIO(index, diordo, bytes(ioList.encode('utf-8')), bytes(ipAddress.encode('utf-8')))
    return message(ret)

def getInterferenceZoneIO(index, diordo, ioList, ipAddress=''):
    strioList = create_string_buffer(1024)
    api_mod.getInterferenceZoneIO.argtypes = [c_int, c_int, c_char_p, c_char_p]
    api_mod.getInterferenceZoneIO.restype = c_int
    ret = api_mod.getInterferenceZoneIO(index, diordo, strioList, bytes(ipAddress.encode('utf-8')))
    if(ret >= 0):
        del ioList[:]
        ioList.append(strioList.value.decode("utf-8"))
    return message(ret)

def delInterferenceZone(index, ipAddress=''):
    api_mod.delInterferenceZone.argtypes = [c_int, c_char_p]
    api_mod.delInterferenceZone.restype = c_int
    ret = api_mod.delInterferenceZone(index, bytes(ipAddress.encode('utf-8')))
    return message(ret)

def controllInterferenceZone(switch, index=-1, ipAddress=''):
    api_mod.controllInterferenceZone.argtypes = [c_int, c_int, c_char_p]
    api_mod.controllInterferenceZone.restype = c_int
    ret = api_mod.controllInterferenceZone(switch, index, bytes(ipAddress.encode('utf-8')))
    return message(ret)

def getInterferenceZoneSwitch(switch, index=-1, ipAddress=''):
    api_mod.getInterferenceZoneSwitch.argtypes = [POINTER(c_int), c_int, c_char_p]
    api_mod.getInterferenceZoneSwitch.restype = c_int
    ret = api_mod.getInterferenceZoneSwitch(byref(switch), index, bytes(ipAddress.encode('utf-8')))
    return message(ret)

def getInterpolaterCycleInterval(interval, ipAddress=''):
    api_mod.getInterpolaterCycleInterval.argtypes = [POINTER(c_double),  c_char_p]
    api_mod.getInterpolaterCycleInterval.restype = c_int
    ret = api_mod.getInterpolaterCycleInterval(byref(interval), bytes(ipAddress.encode('utf-8')))
    return message(ret)
